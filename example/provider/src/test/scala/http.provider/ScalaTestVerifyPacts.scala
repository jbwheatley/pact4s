...
class ScalaTestVerifyPacts extends AnyFlatSpec with BeforeAndAfterAll with PactVerifier {
  ...
  override def beforeAll(): Unit = {
    ...
    locally { val _ = store.create(ProviderResource("missingID", 99)).unsafeRunSync() }
    locally { val _ = store.create(ProviderResource("newID", 66)).unsafeRunSync() }
    ()
  }
      ...
      StateManagementFunction
        .withResponse {
          case ProviderState("resource exists", params) =>
            ...
            (id, value)
              .mapN(ProviderResource.apply)
              .traverse_(store.create)
              .as(Map.empty[String, String])
              .unsafeRunSync()
